...
import { generateAndSaveBill, generateBillPDF } from './bill-generator.mjs';
...
function sendBillDownload(response, orderId, pdfBuffer) {
...
async function handleApi(request, response, url) {
  ...
  if (request.method === 'POST' && url.pathname === '/api/orders') {
    ...
    try {
      ...
      const pdfAvailable = Boolean(billResult.localFilePath);
      const generated = Boolean(billResult.localFilePath || billResult.driveFileId);
      order.bill = {
        generated,
        driveFileId: billResult.driveFileId || null,
        localPath: billResult.localFilePath || null,
        pdfAvailable
      };
  ...
  if (request.method === 'GET' && url.pathname.endsWith('/bill')) {
    ...
    let pdfBuffer = null;
    const localBillPath = order.bill?.localPath;

    if (localBillPath && existsSync(localBillPath)) {
      try {
        pdfBuffer = await readFile(localBillPath);
      } catch (error) {
        console.warn(`Unable to read saved invoice for ${order.orderId}:`, error.message);
      }
    }
    ...
    sendBillDownload(response, order.orderId, pdfBuffer);
    return true;
  }
